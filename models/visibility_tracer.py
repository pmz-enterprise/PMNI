...
class VisibilityTracing(nn.Module):
    ...
    def forward(self,
        ...
        ray_directions = unique_camera_centers.to('cuda').unsqueeze(0) - points.unsqueeze(
    ...
    def sphere_tracing_for_visibility(self, sdf, points, unit_ray_directions, unique_camera_centers):
        ...
        in_progress_mask = torch.ones(num_points, num_cams).to('cuda').bool() # (num_points, num_cams) 512,8
        ...
        current_points_flat = current_points.view(-1, 3)
        current_sdf_flat = sdf(current_points_flat)
        current_sdf = current_sdf_flat.view(num_points, num_cams)  # (num_points, num_cams)
        ...
        while True:
            ...
            current_points_flat = current_points.view(-1, 3)
            masked_points_flat = current_points_flat[in_progress_mask.view(-1)]
            new_sdf_flat = sdf(masked_points_flat)
            current_sdf[in_progress_mask] = new_sdf_flat.squeeze()
